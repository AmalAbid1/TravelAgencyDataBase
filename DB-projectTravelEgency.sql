...
select count(*)
from `Services`;-- NUMBER OF ALL SERVISES
-- ------------------------------------------
CREATE TABLE Seats (
...
AND ID=1;
 ...
 SELECT location, COUNT(*) FROM AirLine WHERE name LIKE '%Airways%' GROUP BY location;
 ...
 SELECT location, COUNT(*) FROM AirLine GROUP BY location ORDER BY COUNT(*) DESC;
 
 SELECT name, AVG(ID_EP) FROM AirLine GROUP BY name ORDER BY AVG(ID_EP) ASC;
...
WHERE
 ...
 SELECT * FROM AirLine WHERE location <> ALL (SELECT location FROM AirLine WHERE name = 'Emirates');
-- -----------------------------------------------------------------------------
...
Note varchar(50),
 ...
 select total_price from Reserva_On;
 update Reserva_On
 ...
 z.total_price
from customer as x
join customer_cost as z
on x.fname = z.Reserva_On;